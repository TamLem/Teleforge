...
3. `apps/web/src/teleforge-generated/client-flow-manifest.ts` exposes browser-safe flow metadata generated by `teleforge generate client-manifest` for screen resolution.
4. `apps/web/src/screens/home.screen.tsx` registers the screen with `defineScreen()`.
5. `apps/web/src/App.tsx` renders the visible Mini App UI with `teleforge/web` hooks and plain React markup.
...
- Regenerate `apps/web/src/teleforge-generated/client-flow-manifest.ts` with `teleforge generate client-manifest` after changing flow route or screen metadata.
