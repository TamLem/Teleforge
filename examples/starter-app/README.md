...
3. `apps/web/src/screens/home.screen.tsx` registers the screen with `defineScreen()`.
4. `apps/web/src/App.tsx` renders the visible Mini App UI with `teleforge/web` and `teleforge/ui`.
5. `apps/bot/src/runtime.ts` exports `createDevBotRuntime()`, which lets `teleforge dev` execute the same flow inside the local simulator chat.
