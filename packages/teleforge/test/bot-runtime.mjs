...
test("createDiscoveredBotRuntime handles requestLocationAction location sharing", async () => {
  const tmpRoot = await mkdtemp(path.join(os.tmpdir(), "teleforge-runtime-location-"));
  const flowsRoot = path.join(tmpRoot, "apps", "bot", "src", "flows");
  const distIndexUrl = pathToFileURL(path.join(process.cwd(), "dist", "index.js")).href;
...
export default defineTeleforgeApp({
  app: {
    id: "runtime-location-app",
    name: "Runtime Location App",
    version: "1.0.0"
  },
  flows: {
    root: "apps/bot/src/flows"
...
`
  ...
  await writeFile(
    path.join(flowsRoot, "location.flow.mjs"),
    `import { chatStep, defineFlow, requestLocationAction } from ${JSON.stringify(distIndexUrl)};
...
export default defineFlow({
  ...
  steps: {
    ask: chatStep("Share your location.", [
      requestLocationAction("Share location", "done", {
        stateField: "coords"
      })
    ]),
    done: chatStep(({ state }) => "Lat " + state.coords.latitude + " Lng " + state.coords.longitude)
  }
});
`
  );
  ...
  assert.equal(sent[0]?.text, "Share your location.");
  assert.equal(sent[0]?.options.reply_markup.keyboard[0][0].request_location, true);
  assert.equal(sent[0]?.options.reply_markup.keyboard[0][0].text, "Share location");
  ...
  assert.equal(sent[1]?.text, "Lat 40.7128 Lng -74.006");
});
